...
//    MainActivity main = new MainActivity();
    ...
    public void onClick(View opButtons){
        ...
        switch (opButtons.getId()){
            ...
            case R.id.next:
                if (clicked){
                    Intent addictionIntent = new Intent(moreUsersActivity.this, moreAddictionActivity.class);
                    Bundle bundle = new Bundle();
                    bundle.putSerializable("ARRAYLIST",(Serializable)drugs);
                    addictionIntent.putExtra("BUNDLE", bundle);
                    addictionIntent.putExtra("drugNames",drugNames);
                    addictionIntent.putExtra("firstname",firstName);
                    addictionIntent.putExtra("lastname", lastName);
                    addictionIntent.putExtra("wins", wins);
                    addictionIntent.putExtra("losses", losses);
                    addictionIntent.putExtra("rounds", rounds);
    ...
    public void findWinner(){
        if (Integer.parseInt(drugs.get(first).getUsers())> Integer.parseInt(drugs.get(second).getUsers()))
            correct = 0;
        else
            correct = 1;
    }
